...
from 
	employee
where 
	title Like '%Manager' and employee_id in (select
                                 reports_to
                                 from employee
                                 group by reports_to
                                having count(*)>=2)
...
from 
	track
left join 
	playlist_track on playlist_track.track_id= track.track_id
left join
	playlist on playlist.playlist_id=playlist_track.playlist_id
where 
	playlist.name like 'Classical%' and composer is not null
group by 
	1;
...
group by 
	customer_id
having 
	bool_and(total_revenue > previous_year_revenue);
